...
| Pixil/Pixilart `.pixil` | Supported subset | Preserves supported layer names, order, visibility, opacity, and RGBA cel pixels when present in the required project structure. | Rebuilds explicit frames and per-frame millisecond durations. | Accepts exactly one JSON project using `pixilart.com/pixil-project`, schema version 1, normal layers, and one full-canvas raw-RGBA cel per layer/frame pair. Alternate schemas, unknown fields, partial or linked cels, external image URLs, non-normal blend modes, masks, groups, effects, palettes, and editor metadata are unsupported. See [pixil-format.md](pixil-format.md). |
