...
| Pixil/Pixilart `.pixil` | Preserves supported direct normal layers only when the required layer data is present. | Explicit frames with one full-canvas cel per layer/frame pair are converted. | Required per-frame millisecond durations are converted and normalized to the Aseprite output range. | Not preserved; effect-related or unknown fields are rejected. | Not preserved. | Not preserved. | Palette data is unsupported; accepted pixels are already RGBA. | Alternate schemas, unknown fields, partial or linked cels, external URLs, and editor metadata are unsupported. Conversion is not guaranteed lossless. |
